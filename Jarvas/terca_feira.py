...
from Jarvas.vscode import VsCode

class TercaFeira(VsCode):
    
    def __init__(self):
        super().__init__()
        self.abrir_vscode()
    
    def abrir_programas_dias_da_semana(self):
        
        # Abre o Spotify nas minhas músicas Favoritas
        os.startfile(TercaFeira.caminho_spotify)
        ...
        TercaFeira.janela_curso_python.wait("exists", timeout=5)
        time.sleep(7)
    ...
    def abrir_programas_final_de_semana():
        os.startfile(TercaFeira.caminho_roblox)
